...
namespace ConfigurationValidation
{
    ...
    namespace akvHelper
    {
        ...
        public static class AzureKeyVaultHelper
        {
            ...
            /// <param name="secretName">name of the secret saved</param>
            /// <returns>secret value</returns>
            public static List<AKVSecretInfo> GetSecrets()
            {
                var vaultClient = new KeyVaultClient(GetToken);
                var secrets = vaultClient.GetSecretsAsync(vaultBaseUrl: VaultAddress).GetAwaiter().GetResult().GetEnumerator();
                var secretList = new List<AKVSecretInfo>();
                while (secrets.MoveNext())
                {
                    var secretName = secrets.Current.Identifier.Name;
                    var secretValue = GetSecretByName(secretName); 
                    secretList.Add(new AKVSecretInfo { KeyName = secretName, KeyValue = secretValue });
                }
